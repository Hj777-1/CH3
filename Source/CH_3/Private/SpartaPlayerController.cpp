...
void ASpartaPlayerController::ShowMainMenu(bool bIsRestart)
{
	...
	if (MainMenuWidgetClass)
	{
		...
		if (MainMenuWidgetInstance)
		{
			...
			UFunction* SetMenuModeFunc = MainMenuWidgetInstance->FindFunction(FName("SetMenuMode"));
			if (SetMenuModeFunc)
			{
				struct FSetMenuModeParams
				{
					bool IsGameOver;
				};
				FSetMenuModeParams Params;
				Params.IsGameOver = bIsRestart;

				MainMenuWidgetInstance->ProcessEvent(SetMenuModeFunc, &Params);
			}

			if (UTextBlock* StartText = Cast<UTextBlock>(MainMenuWidgetInstance->GetWidgetFromName(TEXT("StartButtonText"))))
			{
				StartText->SetText(FText::FromString(bIsRestart ? TEXT("Restart") : TEXT("Start")));
			}
